...
sequenceDiagram
    ...
    GA->>WP: REST API request (retrieve post content)
    WP-->>GA: Post-Daten (HTML)
    GA->>GA: MGenerate Markdown + front matter
    GA->>WP: Download hero piece
    WP-->>GA: Image file
    GA->>Repo: Create a new page folder
    GA->>Repo: Commit & Push
```
Pipeline Description

This pipeline transfers content from a WordPress website into the [cotedi-project](https://github.com/cotedi-project/cotedi-project.github.io.git) GitHub repository automatically — both already existing content (one-time migration) and new posts published going forward (ongoing sync).
...
This notification happens via a repository_dispatch — a GitHub API call that lets you trigger a GitHub Actions workflow from outside the repo itself. [link](https://github.com/marketplace/actions/repository-dispatch)
...
Whether existing or new content, every post ends up in the same consistent format in the repo: a page folder containing a Markdown file and its hero image — no manual copying needed.